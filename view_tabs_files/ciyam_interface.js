...
function load( )
{
   ...
   {
      sessionStorage.setItem( 'app', 'sample' );
...
function hash_password( salt, pwd, skip_id )
{
   ...
   {
      retval = hash_value( serverId + pwd + salt, hashRounds ).toLowerCase( );

      if( !loggedIn && hashRounds != newHashRounds && can_write_to_session_storage( ) )
